...
def maybe_use_lars_optimizer(cfg, optimizer: torch.optim.Optimizer) -> torch.optim.Optimizer:
    ...
    if hasattr(cfg.SOLVER.OPTIMIZER, "LARS") and cfg.SOLVER.OPTIMIZER.LARS.ENABLED:
        eps = cfg.SOLVER.OPTIMIZER.LARS.EPS
        trust_coef = cfg.SOLVER.OPTIMIZER.LARS.TRUST_COEF
        optimizer = LARS(optimizer, eps, trust_coef)
    return optimizer
...
def build_lr_scheduler(
    ...
    scheduler = SCHEDULER_BUILDER.get(name).build(optimizer, cfg)
    return scheduler
